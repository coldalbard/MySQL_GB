...
(2, 'f', '2011-12-20', 13, 'Adriannaport'),
...
-- 1) из одного города
DROP PROCEDURE IF EXISTS identical_cities;
DELIMITER //
CREATE PROCEDURE identical_cities(search_id int)
BEGIN
	SELECT p2.user_id, p2.hometown FROM profiles AS p1
    JOIN profiles AS p2
    ON p1.hometown = p2.hometown
    WHERE p1.user_id = search_id;
END//
DELIMITER ;

-- 2) состоят в одной группе
DROP PROCEDURE IF EXISTS identical_groups;
DELIMITER //
CREATE PROCEDURE identical_groups(search_id int)
BEGIN
	SELECT user_id FROM users_communities
    WHERE community_id IN (SELECT community_id FROM users_communities WHERE user_id = search_id) 
    AND user_id != search_id;
END//
DELIMITER ;
-- 3) друзья друзей
-- надо доделать самому

-- Задача 2. Создание функции, вычисляющей коэффициент популярности пользователя
-- (по заявкам на дружбу – таблица friend_requests)
DROP FUNCTION IF EXISTS application_freinds;
DELIMITER //
CREATE FUNCTION application_freinds(search_user INT)
RETURNS FLOAT READS SQL DATA
BEGIN
    DECLARE req_to_user INT;
    DECLARE req_from_user INT;
    
    SET req_to_user = (SELECT COUNT(*) 
    FROM friend_requests WHERE target_user_id = search_user);
    
    SET req_from_user = (SELECT COUNT(*) 
    FROM friend_requests WHERE initiator_user_id = search_user);
    
    RETURN ROUND(req_to_user / req_from_user * 100, 0);
END //
DELIMITER ;
SELECT application_freinds(1);
