...
SELECT auto.color, auto.mark, count(auto.mark) as CARS_COUNT
FROM auto 
WHERE auto.mark = 'BMW' or auto.mark = 'LADA'
GROUP BY auto.color, auto.mark;

-- 2. Вывести на экран марку авто и количество AUTO не этой марки
SELECT auto.mark, COUNT(auto.mark) as MARK_COUNT
FROM auto
GROUP BY auto.mark;

-- Задание №3. Вывести марку авто - а так же сколько авто такой марки в таблице , вывести записи где количество авто
-- такой марки больше 2, записи не должны дублироваться
SELECT auto.mark, COUNT(auto.mark) as MARK_COUNT 
FROM auto 
GROUP BY auto.mark 
HAVING COUNT(auto.mark) > 2;


-- Задание №4. Вывести Марку и цвет автомобиля - имя и фамилию покупателя, для всех покупателей, 
-- которые живут в городе с населением больше 1 млн человек.
SELECT auto.mark, auto.color, man.firstname, man.lastname FROM auto 
INNER JOIN man
ON auto.phonenum = man.phonenum
WHERE man.citycode IN (SELECT city.citycode FROM city WHERE city.peoples > 1000000);
